function [sys,x0,str,ts] = Series_UCB(t,x,u,flag,params)
% series_ucb
%
% MATLAB Level-1 S-function for real-time load-sharing optimization (RTO)
% of a 2-compressor SERIES network using Upper Confidence Bound (UCB).
    ...
    if SS_flag >= 0.5
        ...
        if dataAdded && nPoints >= params.MinPointsForGP
            ...
            assignin('base','RTO_gps', struct( ...
                ...
                'span', params.T_upper_offset - params.T_lower_offset));
        end

        if nPoints < params.MinPointsForGP
            T_next = chooseNextDOEPoint(DOE_points, Dataset_T, params);
        elseif ~isempty(GP_Power)
            T_next = solveOptimization_UCB_series(Current_setpoint, P_dis_ref, GP_Power, GP_Pdis, GP_ds1, GP_ds2, params);
        end
    end
...
function p = initParams(p)
    ...
    % Series UCB Objective Weights
    if ~isfield(p, 'w_power'), p.w_power = 1; end
    if ~isfield(p, 'w_pdis'), p.w_pdis = 10; end
