function J = objectiveFun_CEI_parallel(T, massflow_ref, GP_Power, GP_Pdis, GP_ds1, GP_ds2, GP_ds3, Dataset_massflowrate, Dataset_cost, params)
    ...
    [mu_g1, ~] = gp_predict(GP_Pdis,  T_row, true); % isP4 = true
    ...
    PoF_arr = max(min(PoF_arr, 1.0), 0.0); % Limit round-off errors to valid probability bounds.
    
    PoF = prod(PoF_arr); % Combined probability

    e = (mu_g1 - massflow_ref) / params.cei_track_tol;
    P_track_g1 = 1 / (e.^2 + 1); % Inverse-quadratic tracking penalty
