function [sys,x0,str,tss]=comp_plant_series_1(t,x,u,flag,Param,X_ss)
...
m_out = Valve_out_gain*Outflow_opening*sqrt(abs(p2 - P_int ))*(p2 - P_int )/abs(p2 - P_int );     % Outflow valve
m_out = max(m_out, 0);
...
eta = 0.60 - (12.0*(m_comp - 0.58).^2 - 4.2*(m_comp - 0.58).*(pr1 - 1.65) + 2.5*(pr1 - 1.65).^2);
...
kappa = 1.27;
T_out = u(6) * (pr1)^((kappa - 1)/(kappa * eta));
...
% valves
m_in = Valve_in_gain * Inflow_opening * sqrt(abs(In_pres - p1)) * sign(In_pres - p1);
m_out_raw = Valve_out_gain * Outflow_opening * sqrt(abs(p2 - P_int)) * sign(p2 - P_int);
m_out = max(m_out_raw, 0);
m_rec_ss = Valve_rec_gain * Recycle_opening * sqrt(abs(p2 - p1)) * sign(p2 - p1);
...
sys(1) = SpeedSound/VolumeT1*(m_in + m_rec - m_comp) ;     %  p1
sys(2) = SpeedSound/VolumeT2*(m_comp - m_rec - m_out) ;    %  p2 
sys(3) = AdivL*(p_comp-p2);                                % m_comp
sys(4) = tauComp*(p_ratio_ss-pr1);                         % p_ratio
sys(5) = 1/J*(torque_drive-torque_comp);                   % omega_comp
sys(6) = tauRecycle*(m_rec_ss - m_rec);                    % m_rec
